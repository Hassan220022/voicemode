...
def _resolve_tts_endpoints(voice, ref_text_override):
    ...
    if clone_profile:
        ...
        if vm_config.OMNIROUTE_ONLY:
            ...
            return list(vm_config.TTS_BASE_URLS), clone_profile
    ...
    return list(vm_config.TTS_BASE_URLS), None
...
async def simple_stt_failover(
    ...
    **kwargs
    ...
    logger.info(f"  Available endpoints: {vm_config.STT_BASE_URLS}")
    if file_size_bytes > 0:
        logger.info(f"  Audio file size: {file_size_bytes / 1024:.1f}KB")

    # Try each STT endpoint in order
    for i, base_url in enumerate(vm_config.STT_BASE_URLS):
        try:
            ...
            if i < len(vm_config.STT_BASE_URLS) - 1:
    ...
    if successful_but_empty:
        ...
        logger.error(f"✗ All STT endpoints failed after {len(vm_config.STT_BASE_URLS)} attempts")
